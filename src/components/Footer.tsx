...
const getPlatformIconByName = (name: string) => {
  ...
  const icons: Record<string, React.ReactNode> = {
    ...
    WhatsApp: (
      <svg className={base} viewBox="0 0 32 32" aria-hidden>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div>
            <Link href={`/${locale}`} className="flex items-center gap-3">
              <div className="relative h-10 w-10 sm:h-11 sm:w-11 shrink-0">
                <Image
                  ...
                  className="object-contain brightness-110 contrast-110 drop-shadow-[0_0_6px_rgba(255,255,255,0.25)]"
                  priority
                />
              </div>
            ...
            <p className="mt-3 text-sm sm:text-base text-white/80">{f.subheading}</p>
          </div>
                ...
                {Object.keys(nav).map((key) => (
                  <li key={key}>
                    <Link
                      ...
                      className="text-white/90 hover:text-emerald-300 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/30 rounded"
                    >
                      {nav[key]}
                    </Link>
                  </li>
              ...
              {Object.keys(footerDetails.socials).map((platformName) => {
                ...
                return (
                  <Link
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-white/90 hover:text-emerald-300 p-2 rounded-full ring-1 ring-white/15 hover:ring-emerald-400/40 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/30"
                  >
