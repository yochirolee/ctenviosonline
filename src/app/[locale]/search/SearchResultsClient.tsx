...
export default function SearchResultsClient({ locale, dict, initialQuery, initialPage }: Props) {
  ...
  const fmt = useMemo(() => new Intl.NumberFormat(locale || 'es', { style: 'currency', currency: 'USD' }), [locale])
  ...
  }, [q, page, location?.country, location?.province, location?.municipality, location?.area_type])
  ...
  const handleAdd = async (p: SimplifiedProduct) => {
    ...
    if (!isLoggedIn) {
      toast.error(dict?.cart?.login_required || (locale === 'en' ? 'You must be logged in to add products to your cart.' : 'Debes iniciar sesión para agregar productos.'))
          ...
          <>
                ...
                <article key={p.id} className="border rounded-xl overflow-hidden shadow-sm bg-white flex flex-col">
                  ...
                  <div className="p-3 flex flex-col grow">
                    ...
                    <div className="mt-auto pt-1">
                      ...
                      >
                        {dict?.cart?.addToCart || (locale === 'en' ? 'Add to Cart' : 'Agregar al carrito')}
                      </button>
                    </div>
                  </div>
                </article>
            ...
            <div className="mt-6 flex items-center justify-center gap-2">
              <button
                ...
                className={`px-3 py-1.5 rounded border text-sm ${page <= 1 ? 'text-gray-400 border-gray-200' : 'hover:bg-gray-50'}`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded border text-sm ${!hasMore ? 'text-gray-400 border-gray-200' : 'hover:bg-gray-50'}`}
              >
