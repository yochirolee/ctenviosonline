...
import { listPartnerOrders, partnerAssign, partnerSetStatus, type PartnerOrder } from '@/lib/partnerApi'
...
export default function PartnerOrdersScreen() {
  ...
  const [status, setStatus] = useState<'paid'|'shipped'|'delivered'>('paid')
  const [scope, setScope] = useState<'mine'|'available'>('mine') // solo aplica a delivery
  ...
  useEffect(() => { load() }, [status, scope, page, limit])
  ...
  const release = async (id: number) => {
    try {
      ...
      await load()
    } catch { toast.error('No se pudo liberar') }
  }
  ...
  const fmtTotal = (n: number | null | undefined) => typeof n === 'number' ? `$${n.toFixed(2)}` : '—'
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => router.back()} className="px-2 py-1 rounded border border-gray-300 bg-white hover:bg-gray-50">←</button>
          <button onClick={() => router.forward()} className="px-2 py-1 rounded border border-gray-300 bg-white hover:bg-gray-50">→</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 bg-white p-4 rounded shadow">
        ...
        {isDelivery && (
          <div>
            ...
            <select className="input" value={scope} onChange={e => { setScope(e.target.value as any); setPage(1) }}>
      ...
      <div className="bg-white rounded shadow divide-y">
        ...
        {!loading && items.length === 0 && <div className="p-4 text-gray-500">Sin órdenes</div>}

        {!loading && items.map(o => {
          const assigneeId = o.metadata?.delivery_assignee_id
          const assignedToMe = String(assigneeId || '') === String(me?.id || '')
          return (
            <div key={o.id} className="p-4 flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-gray-900">Orden #{o.id}</div>
                <div className="text-sm text-gray-700">Cliente: {fmtName(o)} {o.customer_email ? `· ${o.customer_email}` : ''}</div>
                <div className="mt-1 text-xs text-gray-600 space-x-2">
                  <span className="inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-700">Estado: {o.status}</span>
                  <span className="inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-700">Total: {fmtTotal(o.total)}</span>
                  {assigneeId && <span className="inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-700">Asignada a: {o.metadata?.delivery_assignee_name || assigneeId}</span>}
                </div>
              </div>
              ...
              <div className="mt-2 sm:mt-0 flex flex-wrap gap-2 w-full sm:w-auto">
                ...
                {isDelivery && scope === 'available' && !assigneeId && o.status !== 'delivered' && (
                  <button onClick={() => claim(o.id)} className="px-3 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">
                    Tomar orden
                  </button>
                )}
                {isDelivery && assignedToMe && (
                  <>
                    {o.status === 'paid' && (
                      <button onClick={() => mark(o.id, 'shipped')} className="px-3 py-2 rounded bg-indigo-600 text-white hover:bg-indigo-700">
                        Marcar shipped
                      </button>
                    )}
                    {o.status === 'shipped' && (
                      <button onClick={() => mark(o.id, 'delivered')} className="px-3 py-2 rounded bg-green-600 text-white hover:bg-green-700">
                        Marcar delivered
                      </button>
                    )}
                    {o.status !== 'delivered' && (
                      <button onClick={() => release(o.id)} className="px-3 py-2 rounded bg-gray-200">
                        Liberar
                      </button>
                    )}
                  </>
