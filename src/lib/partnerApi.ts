...
export async function listPartnerOrders(params: {
  ...
  return res.json() as Promise<{ items: PartnerOrder[]; page: number; pages: number; total: number; limit: number }>
}

export async function partnerAssign(orderId: number, action: 'take'|'release') {
...
export async function partnerSetStatus(orderId: number, status: 'shipped'|'delivered') {
